...
var configFile = "/var/efw/hotspot/settings";
...
function allowUser(user) {
    exec.execFile('/usr/local/bin/allow_user.sh', ["--allow", user.mac], function(err, stdout, stderr) {
...
exports.token = function(req, res) {
    ...
    generic.sendTokenMessage(user.phonenumber, user.token, function(resp){
        if(resp == 200)
            res.status(200).json({phoneNumber: user.phonenumber});
        else
            res.status(200).json({error: "Sms Send failure"});
    });
}
...
exports.match = function(req, res) {
    ...
    Userdb.tokenCheck(user.uid, req.body.token, function(resp) {
        if(resp)
        {
            allowUser(user);
            res.status(200).json({phoneNumber: user.phonenumber});
            ...
            res.status(200).json({error: "Token mismatch"});
    });
}
...
exports.connect = function(socket) {
    ...
    socket.on("disconnect", function(){
        Userdb.removeWithIp(ip, function(mac){
            if(mac) {
                exec.execFile('/usr/local/bin/allow_user.sh', ["--deny", mac], function(err, stdout, stderr) {
                    console.log("Stdout: " + stdout);
                    console.log("Stderr: " + stderr);
                });
...
exports.userinfo = function(req, res) {
    console.log("USER INFOOOOO: " + req.user.phonenumber);
    var retVal;
    if(req.user.kimlikno != "none")
        retVal = req.user.kimlikno;
    else
        retVal = req.user.phonenumber;
    res.json({username: retVal});
}

exports.getMethod = function(req, res) {
   res.json({authMethod: generic.readConfigKey(configFile, "AUTH_METHOD")});
}
